...
const Admin = () => {
  ...
  const handleSharedLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        'Password accepted, but registrations could not be loaded. The PocketBase collection may require admin access — use admin login instead.'
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="w-14 h-14 bg-green-950 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <Lock className="w-6 h-6 text-yellow-400" strokeWidth={2} />
            </div>
            ...
            {mode === 'admin' ? (
              <form onSubmit={handleAdminLogin} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 rounded-full font-bold text-sm bg-green-600 text-white hover:bg-green-700 active:scale-95 transition-all disabled:bg-gray-300 disabled:text-gray-500"
                >
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-green-950 text-white">
        <div className="container mx-auto px-4 lg:px-8 max-w-6xl py-5 flex items-center justify-between">
          <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="inline-flex items-center gap-3 bg-white border border-gray-100 rounded-2xl px-5 py-3 shadow-sm">
            <div className="w-9 h-9 rounded-xl bg-green-100 flex items-center justify-center">
              <Users className="w-4.5 h-4.5 text-green-700" strokeWidth={2} />
            </div>
            <div>
              <p className="text-2xl font-black text-green-950 leading-none">{records.length}</p>
                ...
                {records.length === 0 ? (
                  ...
                  records.map((r, i) => (
                    <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-gray-500 whitespace-nowrap">{fmtDate(r.created)}</td>
                    </tr>
                  ))
                )}
              </tbody>
